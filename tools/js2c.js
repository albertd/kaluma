...
function generateSources() {
  const template_h = fs.readFileSync(
    __dirname + "/kaluma_modules.h.mustache",
    "utf8"
  );
  const template_c = fs.readFileSync(
    __dirname + "/kaluma_modules.c.mustache",
    "utf8"
  );
  ...
  fs.writeFileSync(path.join(genPath, "kaluma_modules.h"), rendered_h, "utf8");
  fs.writeFileSync(path.join(genPath, "kaluma_modules.c"), rendered_c, "utf8");
}
